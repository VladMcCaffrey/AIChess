...
public class ChessProject extends JFrame implements MouseListener, MouseMotionListener {
    ...
    public ChessProject() {
        ...
        for (int i = 8; i < 16; i++) {
            pieces = new JLabel(loadIcon("WhitePawn.png"));
            panels = (JPanel) chessBoard.getComponent(i);
            panels.add(pieces);
        }

        pieces = new JLabel(loadIcon("WhiteRook.png"));
        panels = (JPanel) chessBoard.getComponent(0);
        panels.add(pieces);

        pieces = new JLabel(loadIcon("WhiteKnight.png"));
        ...
        pieces = new JLabel(loadIcon("WhiteBishop.png"));
        ...
        pieces = new JLabel(loadIcon("WhiteKing.png"));
        panels = (JPanel) chessBoard.getComponent(3);
        panels.add(pieces);

        pieces = new JLabel(loadIcon("WhiteQueen.png"));
        ...
        for (int i = 48; i < 56; i++) {
            pieces = new JLabel(loadIcon("BlackPawn.png"));
            panels = (JPanel) chessBoard.getComponent(i);
            panels.add(pieces);
        }

        pieces = new JLabel(loadIcon("BlackRook.png"));
        panels = (JPanel) chessBoard.getComponent(56);
        panels.add(pieces);

        pieces = new JLabel(loadIcon("BlackKnight.png"));
        ...
        pieces = new JLabel(loadIcon("BlackBishop.png"));
        ...
        pieces = new JLabel(loadIcon("BlackKing.png"));
        panels = (JPanel) chessBoard.getComponent(59);
        panels.add(pieces);

        pieces = new JLabel(loadIcon("BlackQueen.png"));
    ...
    private ImageIcon loadIcon(String filename) {
        try {
            ...
            if (!iconFile.exists()) {
                System.err.println("ERROR: Icon file not found!");
                System.err.println("  Filename: " + filename);
                System.err.println("  Looking for: " + iconFile.getAbsolutePath());
            ...
            if (!iconFile.isFile()) {
                System.err.println("ERROR: Icon path is not a file: " + iconFile.getAbsolutePath());
            ...
            if (icon.getIconWidth() == -1 || icon.getIconHeight() == -1) {
                System.err.println("WARNING: Icon loaded but appears to be empty: " + filename);
                System.err.println("  This may indicate a corrupt or unsupported image format.");
                return icon;
            }

            System.out.println("SUCCESS: Loaded icon: " + filename);
            return icon;

        } catch (Exception e) {
            System.err.println("ERROR: Exception while loading icon: " + filename);
        ...
        for (int i = 0; i < 600; i += 75) {
            for (int j = 0; j < 600; j += 75) {
                ...
                if (tmp instanceof JLabel) {
                    ...
                    // Strip path and .png so we get bare name e.g. "WhitePawn"
                    String iconTmp1 = icon;
                    int sl1 = Math.max(iconTmp1.lastIndexOf("/"), iconTmp1.lastIndexOf("\\"));
                    if (sl1 >= 0) iconTmp1 = iconTmp1.substring(sl1 + 1);
                    if (iconTmp1.toLowerCase().endsWith(".png")) iconTmp1 = iconTmp1.substring(0, iconTmp1.length() - 4);
                    pieceName = iconTmp1;
        ...
        for (int i = 0; i < 600; i += 75) {
            for (int j = 0; j < 600; j += 75) {
                ...
                if (tmp instanceof JLabel) {
                    chessPiece = (JLabel) tmp;
                    icon = chessPiece.getIcon().toString();
                    // Strip path and .png so we get bare name e.g. "BlackPawn"
                    String iconTmp2 = icon;
                    int sl2 = Math.max(iconTmp2.lastIndexOf("/"), iconTmp2.lastIndexOf("\\"));
                    if (sl2 >= 0) iconTmp2 = iconTmp2.substring(sl2 + 1);
                    if (iconTmp2.toLowerCase().endsWith(".png")) iconTmp2 = iconTmp2.substring(0, iconTmp2.length() - 4);
                    pieceName = iconTmp2;
        ...
        if (completeMoves.size() == 0) {
            ...
            if (userChoice == 0) {
                ...
                if (l instanceof JLabel) {
                    ...
                    pieces = new JLabel(loadIcon(startingPoint.getName() + ".png"));
  ...
  public void mousePressed(MouseEvent e) {
    ...
    Component c = chessBoard.findComponentAt(e.getX(), e.getY());
        ...
        if (lastSlash >= 0) pieceName = tmp.substring(lastSlash + 1);
        if (pieceName.toLowerCase().endsWith(".png")) pieceName = pieceName.substring(0, pieceName.length() - 4);
        ...
        System.out.println("white2Move = " + white2Move + " | pieceName = " + pieceName);
        System.out.println("pieceName.contains(Black) = " + pieceName.contains("Black"));
        System.out.println("pieceName.equals(BlackPawn) = " + pieceName.equals("BlackPawn"));
        System.out.println("----------------------------------------------");
            ...
            if (pieceName.contains("Bishop")) {
                ...
                if (startY == 6) { // This is were the pawn is making its first move....
                    ...
                    if (((yMovement == 1) || (yMovement == 2)) && (landingY < startY) && (xMovement == 0)) {
                        if (yMovement == 2) {
                            // Intermediate square is one step above destination (e.getY() - 75)
                    ...
                    // Black pawns move DOWN (increasing Y); promote at row 7
                    if ((yMovement == 1) && (landingY < startY) && (xMovement == 0)) {
                        if (!piecePresent(e.getX(), e.getY())) {
                            validMove = true;
                            /*
                             * Black Pawn reaches the bottom of the board (row 7) and
                             * can now turn into a Black Queen
                             */
                            if (landingY == 0) {
                                progression = true;
                            }
                ...
                if (startY == 1) {
                    if (((xMovement == 0)) && ((yMovement == 1) || ((yMovement) == 2)) && (landingY < startY)) {
                        if (yMovement == 2) {
                            // Intermediate square is one row below destination (between start and dest)
                            if ((!piecePresent(e.getX(), (e.getY()))) && (!piecePresent(e.getX(), (e.getY() + 75)))) {
                                validMove = true;
                            }
                        } else {
                            ...
                            && (landingY < startY)) { // White moves UP so landingY must be less than startY
                        /*
                        ...
                        if (checkWhiteOponent(e.getX(), e.getY())) {
                            validMove = true;
                            if (landingY == 0) { // Promote when reaching top row
                                success = true;
                            }
                            if (getPieceName(e.getX(), e.getY()).contains("King")) {
                                winner = "White Wins!";
                    ...
                    if ((startX - 1 >= 0) || (startX + 1 <= 7)) {
                        if ((piecePresent(e.getX(), e.getY())) && (xMovement == yMovement) && (xMovement == 1)
                                && (landingY < startY)) { // White moves UP so landingY < startY
                            if (checkWhiteOponent(e.getX(), e.getY())) {
                                validMove = true;
                                if (landingY == 0) { // Promote when reaching top row
                                    success = true;
                            ...
                            if (!piecePresent(e.getX(), (e.getY()))) {
                                // White moves UP: landing row must be less than start row
                                if ((xMovement == 0) && (landingY < startY) && (startY - landingY == 1)) {
                                    if (landingY == 0) { // Promote when reaching top row
                                        success = true;
                                    }
                                    validMove = true;
        ...
        if (!validMove) {
            // Invalid move: put the piece back on its original square
            int location = (startY * 8) + startX;
            JPanel originalSquare = (JPanel) chessBoard.getComponent(location);
            // Remove any ghost labels that may have been added
            originalSquare.removeAll();
            chessPiece.setVisible(true);
            originalSquare.add(chessPiece);
            originalSquare.revalidate();
            originalSquare.repaint();
        } else { // this is the condition where we have a valid move and need to visually show
            // it.
            // white2Move will be reset to false by the finally block after makeAIMove()
            white2Move = true; // AI (White) moves next

            if (progression) {
                // Black pawn promotion: replace with Black Queen at destination row
                int location = (landingY * 8) + landingX;
                JPanel destSquare = (JPanel) chessBoard.getComponent(location);
                destSquare.removeAll();
                pieces = new JLabel(loadIcon("BlackQueen.png"));
                destSquare.add(pieces);
                destSquare.revalidate();
                destSquare.repaint();
                if (winner != null) {
                ...
                // White pawn promotion: replace with White Queen at destination row
                int location = (landingY * 8) + landingX;
                JPanel destSquare = (JPanel) chessBoard.getComponent(location);
                destSquare.removeAll();
                pieces = new JLabel(loadIcon("WhiteQueen.png"));
                ...
                // Normal valid move: place piece on destination square
                JPanel destSquare;
                if (c instanceof JLabel) {
                    // Landing on an occupied square (capture) — remove the captured piece
                    Container parent = c.getParent();
                    parent.removeAll();
                    destSquare = (JPanel) parent;
                } else {
            ...
            try {
                ...
                System.err.println("makeAIMove crashed: " + ex.getMessage());
                ex.printStackTrace();
            } finally {
                // Always hand turn back to human (Black) after AI move attempt
                white2Move = false;
                System.out.println(">>> white2Move set to false — Black can now move");
            }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.err.println("Startup makeAIMove crashed: " + ex.getMessage());
            ex.printStackTrace();
        } finally {
            frame.white2Move = false; // Ensure human (Black) can always move first
            System.out.println(">>> Game started. white2Move = false — Black to move");
        }
